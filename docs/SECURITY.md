...
- **Still open:** the composite-foreign-key cross-tenant-safe relation pattern (PRISMA_SCHEMA.md §14.4, DATABASE_DESIGN.md Risk DB-R1) is documented as the mandatory Milestone-4-onward convention (docs/TENANT_ARCHITECTURE.md §4.1) but not yet exercised — no tenant-owned model pair in this milestone's scope (`TenantSettings`/`TenantInvitation`, both referencing only `Tenant`) needs it. **Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** Milestone 4 was rescoped to the salon business-profile domain only; its new tables (`SalonProfile`/`BusinessHours`/`Holiday`) also only reference `Tenant` directly, so this pattern remains dormant through Milestone 4 too. It becomes load-bearing starting with the renumbered Milestone 5's `Employee`↔`Service` relation.
...
- The composite-FK cross-tenant pattern — documented (§3), not yet exercised; no consumer until the renumbered Milestone 5 (docs/adr/ADR-007-salon-management.md — Milestone 4 was rescoped to salon-profile-only and doesn't need it either).
