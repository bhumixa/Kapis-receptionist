...
3. **Salon Management, then catalog data, before Scheduling** — Milestone 4 (the salon's own profile, branding, business hours, and holidays) has no dependency on Employees/Services existing, so it comes first; Milestone 5 (Employees, Services, Customers & Files) then supplies the `Employee`/`Service`/`EmployeeService` data the `Availability` engine and `Appointments` module (DATABASE_DESIGN.md Section 3.5) have a hard dependency on — there is no version of the booking engine that can be meaningfully built or tested against an empty catalog. **Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** this two-step split (profile-only Milestone 4, catalog-data Milestone 5) is narrower/differently-shaped than this document's original single "Salon Management" milestone — see that ADR and the re-scoped Section 3/Section 4 below.
...
7. **Production Deployment last, and deliberately feature-frozen** — Milestone 11 adds no new product features; it exists solely to harden, test, and safely launch what M1–M10 built, preventing the common failure mode where "one more feature" keeps delaying a properly-hardened launch.
...
- **Feature flags, not long-lived branches, isolate incomplete cross-milestone work** (Section 2.2) — e.g., the AI module's tool-calling endpoints (Milestone 8) can exist and be merged to `main` disabled-by-default while Milestone 7 is still being finished, avoiding the integration-debt buildup of a long-running feature branch.

### 1.5 MVP vs. Production Features

This roadmap targets **exactly** PROJECT_REQUIREMENTS.md Section 12's MVP Scope, delivered incrementally across Milestones 1–10, with Milestone 11 dedicated purely to production-readiness (security, performance, monitoring, deployment) rather than new functionality. **Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** the original ten-milestone count is now eleven — the original single "Salon Management" milestone was split into a profile-only Milestone 4 and a catalog-data Milestone 5, cascading every subsequent milestone's number up by one (see Section 3). PROJECT_REQUIREMENTS.md Section 11's Future Features (multi-location/room-based booking, voice channel, in-chat payments, white-labeling, etc.) are **explicitly out of scope for every milestone in this roadmap** — where PRISMA_SCHEMA.md already modeled a future-ready hook for one of these (`Branch`, `Room`, PRISMA_SCHEMA.md Section 5.1), this roadmap's sprints leave those tables present but unused/optional, never spend implementation effort activating them. This boundary is restated here because scope creep during implementation — "while I'm in the Appointments module, let me also wire up Room assignment" — is one of the most common sources of the technical debt and schedule risk this roadmap exists to prevent (Section 10).
...
**Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** the original single "Salon Management" milestone (catalog data — Employees/Services/Customers/Files) is split into two: a profile-only Milestone 4 (salon business profile, branding, business hours, holidays — no dependency on Employees/Services existing) and a new Milestone 5 (the original catalog-data scope, renumbered). Every subsequent milestone's number cascades up by one (old 5→6, 6→7, 7→8, 8→9, 9→10, 10→11).
...
| 4 | **Salon Management** | The salon's own business profile, branding, business hours, and holidays — no dependency on Employees/Services/Customers | 1 | Owner can fully configure the salon profile (business/contact info, timezone, currency, locale, branding placeholder), weekly business hours, and holidays through the dashboard. |
| 5 | **Employees, Services, Customers & Files** | The catalog data every later milestone depends on | 2 | Owner can fully configure Employees, Services, working hours, and Customers through the dashboard; `Files` upload gap (API_SPECIFICATION.md Section 18.2) closed. |
| 6 | **Scheduling Engine** | The core, highest-stakes product capability | 2 | Staff can book, reschedule, and cancel an appointment through the dashboard with real conflict prevention (DATABASE_DESIGN.md Risk DB-R3 closed) and reminders firing on schedule. |
| 7 | **WhatsApp Integration** | Real messages flowing in and out | 2 | A real WhatsApp number receives a real customer message, it's visible in the dashboard inbox, and staff can reply — no AI yet, pure transport + human handling. |
| 8 | **AI Assistant** | The product's core differentiator, live | 2 | A real WhatsApp customer can book, reschedule, or cancel an appointment through an AI conversation, with guardrails, confirmation, and human-escalation all functioning. |
| 9 | **Billing** | The business can collect money | 2 | A tenant can subscribe to a paid plan via Stripe Checkout, see invoices, get blocked appropriately on payment failure, and get unblocked on resolution. |
| 10 | **Analytics, Notifications & Admin** | Insight and oversight surfaces | 2 | Dashboard/Analytics/Reports render real data; email notifications fire for key events; the Super Admin console is functional and provably isolated from tenant-scoped data. |
| 11 | **Production Deployment** | Safe, monitored, real-world launch | 2 | Security review complete, load-tested, deployed on Hetzner behind TLS, monitoring/alerting live, backup/restore drilled, first real pilot tenant live. |

**Total: 19 sprints** (Section 4's detail, Section 11's timeline conversion).
...
> **Execution note (docs/adr/ADR-005-rbac.md):** the RBAC-enforcement slice of this sprint — permission resolver + Redis caching, `RolesGuard`/`PermissionGuard`/`TenantScopedGuard`/`SuperAdminGuard`, `@Roles`/`@RequirePermission`/`@CurrentTenant`, a guard-layer role-rank interpretation, an explicit logged `SUPER_ADMIN` bypass (a reasoned deviation from SYSTEM_ARCHITECTURE.md Section 8.4 — see the ADR), and the frontend `PermissionService`/`roleGuard`/`*appHasPermission`/nav-filtering — was completed under the name "Sprint 2.4," per the requester's narrower brief. **Still open, unchanged from this section's original scope:** the full `Tenants` module (`TenantSettings`, `TenantFeature`), the base-repository/composite-FK cross-tenant pattern (DATABASE_DESIGN.md Risk DB-R1), the standing tenant-isolation regression suite, the atomic `POST /auth/register` transaction rewrite, `TenantActiveGuard` (frontend or backend), the onboarding wizard shell, and any `Users`/`UserRole` CRUD or role-assignment endpoint. A future sprint should pick these up before Milestone 4 begins, since several of Milestone 4's tasks assume the composite-FK pattern exists.
>
> **Execution note (docs/adr/ADR-006-multi-tenant-saas-engine.md):** the remaining items above were completed as "Milestone 3," per the requester's own numbering, with the following deliberate narrowing/differences from this section's original scope (each confirmed with the requester and reasoned in the ADR): **built** — `Tenants` module (`Tenant`, `TenantSettings`, `TenantInvitation` — **not** `TenantFeature`, no requested consumer); `TenantContextService` reworked into the sole authoritative, impersonation-aware tenant-context resolver, fed by a new `TenantMiddleware`; the `TenantScopedRepository` base-repository pattern (composite-FK pattern itself documented as the Milestone-4-onward convention, not yet exercised — no cross-tenant-owned-entity relation exists in this milestone's scope); `POST /auth/register` now atomically creates `Tenant`+`Owner`+**`TenantSettings`** (not `Subscription` — that table still doesn't exist, Milestone 8); `TenantActiveGuard` (backend **and** frontend); a real, persisted, **platform-wide** `AuditLog` (pulled forward from Milestone 9, broader in scope than this section's original tenant-isolation-only framing); Platform Admin tenant switching (impersonation via `X-Impersonate-Tenant-Id`) as a new deliverable beyond this section's original scope. **Not built:** the onboarding wizard shell (not requested this milestone) and any `Users`/`UserRole` staff-CRUD beyond invite-create/list/revoke/accept (`TenantInvitation`'s own lifecycle) — both remain open for a future sprint. See the ADR for full reasoning and docs/TENANT_ARCHITECTURE.md for the as-built reference.
...
> **Execution note (docs/adr/ADR-007-salon-management.md):** built with a narrower and differently-shaped charter than this section's original scope — the requester's explicit brief for this milestone was the salon's own business profile, branding, business hours, and holiday management only, deliberately excluding Employees, Services, Customers, Scheduling, WhatsApp, AI, Billing, and Analytics. The original "Salon Management" milestone's catalog-data scope (Employees/Services/Customers/Files) was moved to a new Milestone 5 below. Executed as a single sprint rather than the two this section originally specified, since the actual scope proved small enough not to need a second.

#### Sprint 4.1 — Salon Profile, Branding, Business Hours & Holidays
- **Objectives:** everything a Salon Owner needs to configure their business's own identity and calendar — no dependency on Employees/Services existing.
- **Tasks:** `Salon` module (`SalonProfile` — description/contact/currency/branding, composed with `Tenant`'s existing profile fields rather than duplicating them, docs/SALON_ARCHITECTURE.md Section 3); `BusinessHours` (weekly template, tenant-wide); `Holiday` (tenant-wide closures, full CRUD); logo upload as a plain placeholder URL field (no `Files`/S3 module — deferred to Milestone 5); frontend `salon` feature (profile page, weekly-hours editor, holidays list with inline add/edit).
- **Deliverables:** `GET/PATCH /salon`, `GET/PUT /salon/business-hours`, `GET/POST/PATCH/DELETE /salon/holidays[/:id]` (API_SPECIFICATION.md Section 6a); a fully click-through-able Salon Profile/Business Hours/Holidays experience on staging.
- **Dependencies:** Milestone 3 (every table here is tenant-owned, built on the Sprint 3.1 pattern; composes with `modules/tenants`' existing `TenantService` rather than duplicating its fields).
- **Estimated Complexity:** Small-Medium — small in raw feature surface (no cross-entity relations, no composite-FK pattern needed yet), the one non-trivial piece being the cross-module transaction composition for `PATCH /salon` (docs/adr/ADR-007-salon-management.md).
- **Risks:** none of this milestone's tables reference another tenant-owned entity, so the composite-FK pattern (PRISMA_SCHEMA.md Section 14.4) remains dormant — it becomes load-bearing starting Milestone 5's `Employee`↔`Service` relation, not here.
- **Acceptance Criteria:** an Owner can view and edit their salon's profile/branding, set a full 7-day business-hours schedule, and add/edit/delete holidays, entirely through the dashboard UI, with tenant isolation verified in integration tests.

### Milestone 5 — Employees, Services, Customers & Files

> **Execution note (docs/adr/ADR-007-salon-management.md):** this milestone is the original "Salon Management" milestone's catalog-data scope, renumbered — `BusinessHours` moved to Milestone 4 (it's tenant-wide, not employee-specific, and has no dependency on `Employee` existing). Sprint numbers below are renumbered from this section's original 4.1/4.2 accordingly.
>
> **Execution note (docs/adr/ADR-008-workforce-and-service-catalog.md):** built with a narrower charter than this section's scope — **built:** `Employees` module (`Employee`, `WorkingHours`, `EmployeeService`) and `Services` module (`ServiceCategory`, `Service`), including employee status, working hours (split shifts), employee ↔ service assignment, and a new per-employee `EmployeeTimeOff` model (date-range leave — not this section's `EmployeeAvailability` design; see docs/DATABASE_DESIGN.md §3.3.7's note) and a new `Service.bufferTimeMinutes` field (per-service buffer, distinct from the tenant-wide `TenantSettings.business.bookingBufferMinutes`). The `EmployeeService` junction is the first real exercise of the composite-FK cross-tenant pattern (docs/TENANT_ARCHITECTURE.md §4.1) — confirmed to need **no manual migration SQL** with this project's Prisma version, correcting `PRISMA_SCHEMA.md` §14.4's original assumption. **Not built, explicitly deferred:** `Customers` module (`Customer`, `CustomerTag`, `CustomerNote`, `CustomerPreference`, `CustomerTagAssignment`) and the `Files`/S3 pre-signed-upload module (`API_SPECIFICATION.md` §18.2's flagged gap) — both remain open for a future milestone, per the requester's explicit brief for this pass. The `EMPLOYEE_HAS_UPCOMING_APPOINTMENTS` guardrail flagged below as a Sprint 5.1 risk could not be implemented even against an empty table as originally suggested, since no `Appointment` table exists in the schema at all yet — genuinely deferred to Milestone 6, not silently skipped. See docs/WORKFORCE_ARCHITECTURE.md and docs/SERVICE_ARCHITECTURE.md for the as-built reference.

#### Sprint 5.1 — Staff & Catalog Backend
- **Objectives:** every piece of structured data the booking engine (Milestone 6) and the AI (Milestone 8) will depend on.
- **Tasks:** `Employees` module (`Employee`, `WorkingHours`, `Holiday` per-employee override, `EmployeeAvailability`); `Services` module (`ServiceCategory`, `Service`, `EmployeeService`); **close the file-upload gap** flagged in API_SPECIFICATION.md Section 18.2/FRONTEND_ARCHITECTURE.md Section 18.7 — design and implement `POST /files` (pre-signed S3 upload URL issuance) plus the `Files` module, needed for real salon logo upload (superseding Milestone 4's placeholder `logoUrl` string) and, later, WhatsApp media (Milestone 7).
...
- **Dependencies:** Milestone 3 (every table here is tenant-owned, built on the Sprint 3.1 pattern). **Implement the composite-FK cross-tenant pattern here** (PRISMA_SCHEMA.md Section 14.4) for `EmployeeService`'s `Employee`↔`Service` relation — its first real consumer, per docs/TENANT_ARCHITECTURE.md Section 4.1/docs/adr/ADR-007's forward note.
- **Estimated Complexity:** Medium.
- **Risks:** the `EMPLOYEE_HAS_UPCOMING_APPOINTMENTS` guardrail (API_SPECIFICATION.md Section 7) can't be *tested* meaningfully until Appointments exists (Milestone 6) — implement the check now against an empty `Appointment` table, add a regression test for it explicitly once Milestone 6 lands, don't let it be silently forgotten. Also: `Holiday`'s Milestone 4 constraint (`@@unique([tenantId, date])`) must be replaced here with a manual partial unique index (`WHERE employee_id IS NULL`) once `employeeId` is added — see docs/SALON_ARCHITECTURE.md Section 2.4's forward note.
- **Acceptance Criteria:** an Owner can create a service, mark an employee eligible for it, and set that employee's weekly working hours, entirely through API calls verified in integration tests (frontend lands in Sprint 5.2).

#### Sprint 5.2 — Customers Backend & Catalog Frontend
- **Objectives:** complete the Employees/Services/Customers domain and give it a real UI, closing out the onboarding wizard's data-entry steps.
...
- **Estimated Complexity:** Medium.
- **Risks:** onboarding-wizard scope creep (it's tempting to make this feel "finished" before WhatsApp/AI steps exist to append to it) — scope this sprint's wizard work strictly to what Milestone 5's data supports, revisit in Milestones 7/8.
...
> **Execution note (docs/adr/ADR-009-scheduling-engine.md):** executed as a single pass covering both sprints below, per the requester's brief. **Built, matching this section's scope:** `Availability` module, `Appointments` module (`Appointment`/`AppointmentService`/`AppointmentStatusHistory`), the two-layer conflict-prevention mechanism (Redis lock + `EXCLUDE` constraint, verified with the exact scripted concurrency test this section's acceptance criteria specify), `POST/GET/PATCH/DELETE /appointments[/:id]`, `.../cancel`, `.../reschedule`, `GET /appointments/availability`, the generic `Idempotency-Key` interceptor (pulled forward from Sprint 6.2, landing here since this milestone's own endpoints are its natural first consumer), and the full frontend `appointments`/`customers` features (calendar Day/Week views, drag-and-drop reschedule, booking form, availability picker). **Confirmed-with-requester deviation:** per-service employee assignment — a single visit's services may each be performed by a different employee, not one employee per appointment — the single largest design decision beyond this section's original scope, with real schema/API consequences (see the ADR). `Customers` (`modules/customers`, deferred out of Milestone 5) was also built this milestone, since Appointments needs it. **Not built, explicitly deferred:** `AppointmentReminder`/BullMQ reminder scheduling (no notifications beyond scheduling were requested) and `AppointmentFeedback` — both flagged in the ADR, not silently dropped. See docs/SCHEDULING_ARCHITECTURE.md and docs/CALENDAR_ENGINE.md for the full as-built reference.

#### Sprint 6.1 — Availability Engine & Booking Conflict Prevention
...
> **Execution note (docs/adr/ADR-010-whatsapp-platform.md):** built as a single `modules/whatsapp` NestJS module (not the three separate `Conversations`/`Messages`/`WhatsApp` modules this section and SYSTEM_ARCHITECTURE.md originally sketched), layered per aggregate internally. **Built:** `WhatsAppAccount` connect/disconnect (credential-verified against Meta before persisting), webhook signature verification + a global `WebhookEvent` ingestion log, two BullMQ queues (`whatsapp-inbound`/`whatsapp-outbound` — this codebase's first job-queue consumer, `src/queues/` was previously an empty placeholder), `Conversation`/`Message` persistence with two-layer idempotency (Redis dedup + DB partial-unique on `whatsappMessageId`, mirroring ADR-009's booking-conflict precedent), contact synchronization (a new `CustomerService.findOrCreateByPhoneForTenant` method on the existing Milestone 6 Customers module), manual outbound replies with 24-hour-window enforcement, and a two-pane frontend inbox. A new `core/security/encryption.service.ts` (AES-256-GCM) encrypts the tenant's WhatsApp access token at rest — this codebase's first *decryptable* stored secret. **Not built, explicitly deferred:** `TemplateMessage` registry (outbound past the 24h window is rejected, not template-routed), actual media file download/S3 storage (metadata only — Meta's webhook payload already includes media id/mime-type/sha256/filename, so no extra API call was needed; the `Files`/S3 module itself remains open from Milestone 5), and any AI/escalation state on `ConversationStatus` (`OPEN`/`RESOLVED`/`CLOSED` only — Milestone 8's job). See docs/WHATSAPP_ARCHITECTURE.md and docs/MESSAGING_ARCHITECTURE.md for the as-built reference.

#### Sprint 7.1 — Webhook Ingestion, Conversations & Messages
...
#### Sprint 7.2 — Outbound Messaging, Media, Templates & Inbox Frontend
- **Objectives:** two-way, human-operable WhatsApp communication through the dashboard — the full product loop minus AI.
- **Tasks:** `TemplateMessage` registry; `Media` handling (inbound download + S3 storage, outbound upload, SYSTEM_ARCHITECTURE.md Section 6.8); `POST /messages/send` (staff manual reply, with `Idempotency-Key`, reusing Sprint 6.2's generic idempotency mechanism); frontend `conversations` feature — the two-pane inbox, message thread, composer (FRONTEND_ARCHITECTURE.md Section 6.6).
- **Deliverables:** a staff member can view an inbound WhatsApp conversation and reply to it through the dashboard, entirely without AI involvement — this is a genuinely useful, shippable capability on its own (a human-operated WhatsApp inbox), deliberately valuable as a standalone increment even before Milestone 8 adds AI on top.
...
> **Execution note (docs/adr/ADR-011-ai-receptionist.md):** built as a single `modules/ai` NestJS module orchestrating the *existing* Appointments/Availability/Services/Salon/Tenants/WhatsApp module services as tools, per this section's own "AI orchestrates, never duplicates" framing. Both sprints below shipped together as one milestone rather than as two incremental releases (the read-only tools and the mutating/guardrail tools share the same orchestrator/tool-executor/prompt-builder infrastructure, and splitting them into separate deploys would have meant shipping a conversational AI that could answer questions but not book — a materially incomplete product surface). **Built:** all seven tools (`checkAvailability`, `bookAppointment`, `rescheduleAppointment`, `cancelAppointment`, `recommendService`, `answerFaq`, `escalateToHuman`), the OpenAI tool-calling loop with Structured Outputs, `AIContext`/`ConversationSummary`/`PromptVersion`, the confirmation-before-booking guardrail (prompt-level) plus server-side re-validation of every tool argument (the actual security boundary), the fallback/auto-escalation path for OpenAI outages, human hand-off (`ConversationStatus.ESCALATED`, `PATCH /conversations/:id/assign`), and the dashboard "Test my AI" sandbox. **Deviated:** `checkAvailability`/`recommendService` have no dedicated internal HTTP endpoint (in-process only — the other five tools' endpoints cover the testable-surface requirement); the repeated-clarification-loop threshold substitutes for a true model-confidence score (none is exposed by the Chat Completions API); no streaming (no SSE/WebSocket transport exists in this codebase to build it on). **Real-usage cost/latency data gathering** (this section's own Sprint 8.1 risk note, informing Milestone 9's plan-limit tuning) remains an operational activity for staging/production rollout, not something a local-dev milestone close can produce. See docs/AI_ARCHITECTURE.md, docs/TOOLS.md, docs/PROMPT_ENGINEERING.md for the as-built reference.

#### Sprint 8.1 — AI Orchestration Foundation & Read-Only Tools
...
- **Dependencies:** Milestones 6 and 7 both complete (this is the AI's first point of contact with real booking data and real WhatsApp transport, per Section 1.1's dependency reasoning).
- **Estimated Complexity:** Large — first LLM-integration sprint, expect iteration on prompt reliability.
- **Risks:** OpenAI response latency/cost under real usage patterns is genuinely hard to predict from documentation alone — this sprint should include real, logged usage against the sandbox WhatsApp number specifically to gather latency/token-cost data informing Milestone 9's plan-limit tuning.
...
> **Execution note (docs/adr/ADR-012-billing-and-subscriptions.md):** both sprints below shipped together as one milestone rather than as two incremental releases, mirroring Milestone 8's own precedent — a Checkout/webhook path with no frontend to drive it, or a frontend with no working enforcement behind it, would each have been a materially incomplete product surface on their own. **Built:** everything Sprint 9.1/9.2 below list, plus `POST /subscriptions/{change-plan,cancel,reactivate,portal-session}` and `GET /payments` — a richer action-endpoint surface than the original single-`POST /subscriptions` design anticipated (docs/API_SPECIFICATION.md §13's amendment note), and `GET/PATCH /admin/plans[/:id]` + `GET /admin/tenants/:id/billing` (Admin module, not originally scoped to this milestone but a necessary support-tooling gap-fill, same category as Milestone 8's `PATCH /conversations/:id/assign`). **Deviated:** the dunning flow (failed-payment `Notification`, retry-window handling) was **not built** — `PAST_DUE` uses a grace-period policy instead (stays fully functional; only Stripe's own dunning-retries-exhausted `canceled`/`unpaid` states block the tenant), an approved, narrower alternative to Sprint 9.2's original "dunning + suspension" framing, since no `Notifications`-triggering mechanism existed yet to build a real dunning email on top of. Plan tiers are 3 illustrative, placeholder-priced tiers (Starter/Professional/Business), not real Stripe Prices — no live Stripe account exists for this project. Plan changes are immediate-with-proration for both upgrade and downgrade, one code path, rather than a deferred-to-period-end design. Sprint 9.1's webhook-ordering risk note was addressed directly: every webhook handler (subscription/invoice/payment) is independently idempotent via upsert-by-Stripe-id, so replay/reordering under BullMQ's at-least-once redelivery is safe by construction, not just tested against. See docs/BILLING_ARCHITECTURE.md, docs/STRIPE_INTEGRATION.md, docs/FEATURE_ENTITLEMENTS.md for the as-built reference.

#### Sprint 9.1 — Stripe Integration & Plan Enforcement
- **Objectives:** the platform can accept real payment and enforce real plan limits.
- **Tasks:** `Plan`/`Subscription`/`Invoice`/`Payment`/`Coupon` modules; Stripe Checkout session creation (`POST /subscriptions`); Stripe webhook handling + signature verification (`POST /stripe/webhook`, `WebhookLog`); usage-limit enforcement wired into the AI module's rate limiting (`messagesUsedCurrentPeriod`, FR-22) — connecting Milestone 8's AI usage directly to this sprint's billing state, closing the loop SYSTEM_ARCHITECTURE.md Section 5.7 flagged as a cost-control necessity.
...
- **Dependencies:** Milestone 3 (Subscription already exists in `TRIALING` state from registration), Milestone 8 (usage tracking has something real to meter).
...
#### Sprint 9.2 — Dunning, Billing Frontend & Tenant-Suspension Enforcement
...
### Milestone 10 — Analytics, Notifications & Admin

#### Sprint 10.1 — Dashboard Aggregation, Audit Trail Completion & Notifications
...
- **Dependencies:** Milestones 3–9 (this milestone is deliberately, entirely derivative of prior data).
...
- **Acceptance Criteria:** a Super Admin (once Sprint 10.2 lands the console) or a direct database query can reconstruct a complete, correct history of any given appointment or subscription purely from `AuditLog`/`AppointmentStatusHistory`.

#### Sprint 10.2 — Reports, Frontend & Super Admin Console
...
- **Dependencies:** Milestone 10 (feature-complete system to actually test).
...
- **Acceptance Criteria:** a full backup-then-restore cycle is executed against a copy of the production database and verified byte-correct; a security scan report shows no unresolved high/critical findings; the booking-conflict concurrency test passes at 10x Sprint 6.1's original test load.

#### Sprint 11.2 — Go-Live
- **Objectives:** a real salon, live, on the real platform.
- **Tasks:** production data migration dry-run (empty-database migration verified against the exact production migration order, PRISMA_SCHEMA.md Section 14.2); finalize real WhatsApp Business number cutover (contingent on Sprint 7.1's verification having cleared by now — Section 10 tracks this explicitly); execute a go-live checklist (DNS, TLS certificate validity, environment variable audit, monitoring dashboards live); onboard one real pilot tenant; a defined post-launch monitoring window (active, elevated attention for the first 1–2 weeks of real traffic, not a "ship and walk away" moment); finalize all seven prior documents (including docs/SALON_ARCHITECTURE.md) to reflect their final as-built state (Section 8).
- **Deliverables:** a real, paying (or trial) salon successfully using the live product; all documentation current.
- **Dependencies:** Sprint 11.1.
...
**Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** `Salon` is a new module inserted at position 5 (Sprint 4.1), and every module previously numbered 5+ shifts down accordingly. `Files` no longer needs to exist for `Tenants`' `logoFileId` this early — Milestone 4's `SalonProfile.logoUrl` is a plain placeholder string, so `Files` is now purely a Milestone 5 concern (needed by `Employees`'/`Services`' onboarding-adjacent uploads and, later, `WhatsApp`'s `Media`).
...
| 5 | `Salon` | 4.1 | First tenant-owned domain module built — depends only on `Tenants` (composes its profile fields via `TenantService`), no dependency on Employees/Services/Customers. |
| 6 | `Employees` | 5.1 | First tenant-owned domain module with per-row CRUD, and a direct dependency of `Services` (eligibility) and `Availability` (scheduling). |
| 7 | `Services` | 5.1 | Depends on `Employees` (`EmployeeService` eligibility); direct dependency of `Appointments`. |
| 8 | `Files` | 5.1 | Needed by `Employees`/`Services`-adjacent uploads and, later, `WhatsApp`'s `Media` — built once its own dependencies exist, ahead of its consumers. Supersedes Milestone 4's placeholder `SalonProfile.logoUrl` string once available. |
| 9 | `Customers` | 5.2 | Depends on `Tenants`; direct dependency of `Appointments` and `Conversations`. |
| 10 | `Availability` | 6.1 | Depends on `Employees`/`Services`/existing `Appointments`; must exist before `Appointments` can enforce conflict prevention. |
| 11 | `Appointments` | 6.1–6.2 | The core scheduling engine — depends on `Customers`/`Employees`/`Services`/`Availability`, all of which now exist. |
| 12 | `WhatsApp` | 7.1–7.2 | Depends on `Tenants` (number-to-tenant resolution) and `Customers` (`findOrCreateByPhone`); its `Media` sub-concern depends on `Files`. |
| 13 | `Conversations` / `Messages` | 7.1–7.2 | Depend on `WhatsApp`'s transport layer and `Customers`. |
| 14 | `AI` | 8.1–8.2 | The last module to be built precisely because it depends on **all** of `Appointments`, `Availability`, `Services`, `Employees`, `Customers`, `Conversations`, and `WhatsApp` being real and working (SYSTEM_ARCHITECTURE.md Section 5.3) — building it earlier would mean building against stubs. |
| 15 | `Billing` | 9.1–9.2 | Depends on `Tenants`; deliberately sequenced after the product has real value to sell (Section 1.1) even though it has no *technical* dependency on `AI`/`WhatsApp`. |
| 16 | `Notifications` (full build-out) | 10.1 | Depends on `Users`/`Billing`(failure events)/`Tenants`(invitations) — its minimal email-sending capability was pulled forward to Sprint 2.1 as a dependency of `Auth`'s verification flow, a deliberate, documented exception to strict dependency ordering (small, low-risk, unblocks Milestone 2). |
| 17 | `Dashboard` / `Analytics` | 10.1 | Purely compositional (SYSTEM_ARCHITECTURE.md Section 3.2) — cannot meaningfully exist before the modules it composes from (`Appointments`, `Conversations`, `Billing`) do. |
| 18 | `Admin` | 10.2 | Deliberately last among functional modules — cross-tenant by design, built once every tenant-scoped module it needs to oversee already exists and is stable. |
| 19 | `AuditLogs` / `ActivityLog` (as a cross-cutting concern) | Incrementally, every sprint from 3.1 onward; verified complete in 10.1 | Not a single-sprint module — every module from `Tenants` onward is required to write its own audit entries as it's built (SYSTEM_ARCHITECTURE.md Section 9.6), with Sprint 10.1 serving as the scheduled completeness audit, not the first time audit logging is considered. |

---

## 6. Frontend Module Order

Restating FRONTEND_ARCHITECTURE.md Section 18.5's 14 lazy-loaded feature modules as a build sequence, mirroring Section 5's backend order exactly (each frontend feature is built in the same sprint as, or the sprint immediately following, its backing API). **Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** a new `salon` feature is inserted at Sprint 4.1; every module previously numbered 3+ shifts one sprint later than this table's original numbering (`employees`/`services`/`customers` now Sprint 5.2, not 4.2, etc.).
...
| 2 | `onboarding` | 2.2 (shell) → 5.2 (data steps) → 8.2 (AI step) | Built incrementally alongside whichever backend capability its current step configures — deliberately the one feature module *not* built in one sprint, since its purpose is literally to walk through the others in sequence. |
| 3 | `salon` | 4.1 | Depends only on the `Salon` API (Sprint 4.1) — profile/branding/business-hours/holidays, no dependency on Employees/Services existing. |
| 4 | `employees` | 5.2 | Depends on the `Employees` API (Sprint 5.1). |
| 5 | `services` | 5.2 | Depends on the `Services` API (Sprint 5.1); built alongside `employees` since both power the same onboarding steps and share the `EmployeePicker`/`ServicePicker` domain components (FRONTEND_ARCHITECTURE.md Section 1.6). |
| 6 | `customers` | 5.2 | Depends on the `Customers` API (Sprint 5.2). |
| 7 | `appointments` | 6.2 | Depends on `Appointments`/`Availability` (Sprint 6.1) — the single largest, most interaction-rich feature module (calendar, drawer-based booking form), matching its backend's own Extra-Large complexity rating. |
| 8 | `conversations` | 7.2 | Depends on `Conversations`/`Messages`/`WhatsApp` (Sprint 7.1) — includes the AI-context side panel built in anticipation of, but functional ahead of, Milestone 8's AI module actually populating it. |
| 9 | `billing` | 9.2 | Depends on `Billing`/Stripe integration (Sprint 9.1); the `TenantActiveGuard` this module's routing depends on (FRONTEND_ARCHITECTURE.md Section 3.3) was structurally built back in Sprint 3.1, activated fully here. |
| 10 | `dashboard-home` | 10.2 | Compositional, depends on `Dashboard` API (Sprint 10.1) — deliberately one of the last feature modules despite being the first *page* a logged-in user sees, since it has nothing real to aggregate until the modules it summarizes exist. |
| 11 | `analytics` | 10.2 | Same reasoning as `dashboard-home`, one sprint later since it depends on a larger backing dataset (`GET /analytics`'s date-range trend data) to be meaningfully testable. |
| 12 | `notifications` | 10.2 | Depends on the full `Notifications` build-out (Sprint 10.1) rather than Sprint 2.1's minimal email-only version. |
| 13 | `settings` | 3.1 (tenant profile) → Milestone 8 (AI behavior fields, populated once `TenantSettings` fields have real consumers) | Built incrementally like `onboarding` — the `Tenant`/`TenantSettings` API exists from Sprint 3.1, but fields like `aiGreetingMessage` are only meaningfully editable once Milestone 8's AI actually reads them. |
| 14 | `profile` | 2.2 | Depends only on `Auth`/`Users` (Sprint 2.1) — built early since it's a small, low-risk module and a natural pairing with the auth work already in flight that sprint. |
| 15 | `admin` | 10.2 | Last, matching its backend's own last-built position (Section 5) — deliberately never built alongside any tenant-scoped feature module, keeping its bundle and its authorization model cleanly separate throughout development, not just at the architecture-document level (FRONTEND_ARCHITECTURE.md Section 14.2). |
...
| **4. Salon Management** | DTO validation rules (business-hours time-range/day-set rules, phone/currency/color format) | CRUD endpoints per module against test DB; cross-tenant isolation for salon profile/business-hours/holidays | Owner configures salon profile, branding, business hours, and holidays, full click-through | N/A | N/A | Tenant-scoped authorization test (STAFF read-only, MANAGER+ for mutations) |
| **5. Employees, Services, Customers & Files** | DTO validation rules (duration/price bounds, phone format) | CRUD endpoints per module against test DB; file-upload flow against a test S3 bucket; first exercise of the composite-FK pattern (`EmployeeService`) | Owner configures a service + employee + customer, full click-through | Onboarding-wizard usability pass | N/A | File-upload content-type/size validation abuse testing |
| **6. Scheduling Engine** | Availability-computation logic (working hours + holidays + buffers) | **Concurrency test** (Section 4, Sprint 6.1) — scripted parallel booking-conflict test against real Postgres + Redis | Book → reschedule → cancel, full click-through | Manual verification of reminder timing across a DST boundary date | **Load test the booking-critical path specifically** (SYSTEM_ARCHITECTURE.md's Critical NFR) | Authorization-scoping test (Staff cannot act on another employee's appointment) |
| **7. WhatsApp Integration** | Webhook payload normalization logic | Webhook signature verification (valid/invalid/replayed payloads); idempotency-dedup test (same `whatsappMessageId` twice) | Real WhatsApp device round-trip (send message, see it in dashboard, reply, see it delivered) | Manual real-device testing across at least two WhatsApp client versions (iOS/Android) | Webhook burst-handling test (many rapid inbound events) | Signature-forgery rejection test; media-upload content-type validation |
| **8. AI Assistant** | Prompt-template variable interpolation, guardrail validation functions | Tool-call execution against real `Appointments`/`Availability` (never mocked, per Section 4 Sprint 8.2) | Full real-WhatsApp AI booking conversation, multiple scenarios (happy path, ambiguous request, escalation) | **Adversarial manual testing** — an explicit, scripted attempt to confuse/mislead the AI into an incorrect action | AI response-latency measurement under concurrent conversations | Guardrail-bypass adversarial testing (hallucination/injection attempts via crafted customer messages) |
| **9. Billing** | Plan-limit calculation logic | Stripe webhook handling via Stripe CLI event replay (including out-of-order delivery) | Full Stripe Checkout test-mode flow, subscription change, invoice view | Manual dunning-flow walkthrough (simulate failed card in Stripe test mode) | N/A | Webhook signature-forgery rejection test (mirroring Milestone 7's WhatsApp equivalent) |
| **10. Analytics, Notifications & Admin** | Aggregation/formatting logic | `Admin` cross-tenant read verified; tenant-scoped-token-rejected-by-admin-endpoint verified (bidirectional, Section 4 Sprint 10.2) | Dashboard/Analytics render correctly with real seeded data; Admin console walkthrough | Manual audit-log completeness spot-check against Section 4 Sprint 10.1's audit pass | Dashboard/Analytics query performance against a realistically-sized seeded dataset | Admin-role-escalation attempt test (a `STAFF` token attempting any `/admin/*` call) |
| **11. Production Deployment** | N/A (no new logic) | Full regression run of every prior milestone's integration suite against the production-configured stack | Full regression run of every prior milestone's E2E suite against staging | **Backup/restore drill** (Section 4 Sprint 11.1); go-live checklist execution | **Full load test** at realistic multi-tenant concurrent scale; the Milestone 6 concurrency test re-run at 10x | **Full OWASP-checklist security review**; dependency vulnerability scan; TLS/header verification against the live domain |
...
| 4. Salon Management | new: `docs/SALON_ARCHITECTURE.md`, `docs/adr/ADR-007-salon-management.md`; API_SPECIFICATION.md (new Section 6a `Salon` endpoints), DATABASE_DESIGN.md (new `salon_profiles`/`business_hours` sections, `holidays` amended) | The salon-profile domain gets its own as-built reference and ADR, per docs/adr/ADR-007-salon-management.md's own decision record. |
| 5. Employees, Services, Customers & Files | API_SPECIFICATION.md (add `POST /files` and any `ServiceCategory` endpoints — closing Section 18.2's flagged gaps with the actual, now-real contract) | The gap-closure endpoints get properly specified, not just implemented ad hoc. |
| 6. Scheduling Engine | DATABASE_DESIGN.md (Risk DB-R3 marked closed, `EXCLUDE` constraint documented as implemented), PRISMA_SCHEMA.md (any refinement to the conflict-prevention migration) | |
| 7. WhatsApp Integration | SYSTEM_ARCHITECTURE.md (any refinement to the webhook/queue design based on real Meta API behavior encountered) | Third-party integrations are the most likely place reality diverges from documentation-stage assumptions — this is expected, and the doc is updated to match reality, not treated as wrong. |
| 8. AI Assistant | SYSTEM_ARCHITECTURE.md (prompt versioning specifics, guardrail implementation details refined against real OpenAI behavior) | |
| 9. Billing | API_SPECIFICATION.md (any Stripe-webhook-driven refinement) | |
| 10. Analytics, Notifications & Admin | API_SPECIFICATION.md (`ServiceCategory` if not closed earlier; any Admin-endpoint refinement) | |
| 11. Production Deployment | **All seven documents (including docs/SALON_ARCHITECTURE.md) reviewed and finalized to as-built status**; new: `CHANGELOG.md` (full history), `docs/runbooks/` (deploy, rollback, incident-response, backup-restore) | The formal "documentation matches reality" checkpoint before go-live — not the first time documents are touched, but the point where they're verified complete. |
...
|---|---|---|---|
| IR-1 | **WhatsApp Business Account verification delay** (Meta's external review, days to weeks, outside developer control) | High — could block Milestone 7/11's real-number cutover | Submitted in Sprint 1.1, tracked continuously, sandbox-number development path (Section 4, Sprint 7.1) keeps the critical path unblocked regardless of verification timing. |
...
| IR-4 | **OpenAI API cost/behavior drift during development** (prompt iteration burning real API spend before Milestone 9's usage tracking exists) | Medium — a real, if bounded, cost risk during Milestone 8 | Set a hard development-phase OpenAI spend alert/cap independent of the production billing system; Sprint 8.1 explicitly gathers real cost data specifically to inform Milestone 9's plan-limit tuning, turning a risk into planned learning. |
| IR-5 | **Stripe test-mode/live-mode behavioral divergence** (Section 4, Sprint 9.1) | Medium — a common, well-documented class of late-discovered billing bug | Explicit live-mode-configuration verification task in Sprint 9.1 even though development runs in test mode; Stripe CLI webhook-replay testing rather than manual-only testing. |
| IR-6 | **Booking-conflict-prevention regression** introduced by a later, unrelated change (any module touching `Appointment` after Milestone 6) | Critical — this is the platform's single worst possible defect class (Section 4, Sprint 6.1) | The Sprint 6.1 concurrency test becomes a **standing, permanent CI regression gate** (mirroring Sprint 3.1's tenant-isolation suite) — re-run on every PR touching the `Appointments` module for the rest of the project, not just once. |
...
| IR-8 | **Documentation drift** (the seven source documents, including docs/SALON_ARCHITECTURE.md, silently falling out of sync with the real, implemented system) | Medium — compounds over time, eventually undermining the entire documentation-first approach this project is built on | Section 8.1's hard PR-level policy, enforced as a literal Section 13 quality gate, not just a stated intention. |
| IR-9 | **Timezone/DST edge-case bugs** (reminders, availability, booking windows — flagged repeatedly across Sprints 6.2, 7.2) | Medium — subtle, hard-to-notice-until-a-customer-complains class of bug | Explicit DST-boundary test cases required wherever a sprint's acceptance criteria already calls this out (Section 4); DATABASE_DESIGN.md Section 1.9's UTC-storage discipline is the structural mitigation, but the *scheduling logic* built atop it still needs its own tests. |
| IR-10 | **AI guardrail bypass leading to an incorrect real-world booking action** (the platform's top product-trust risk) | Critical | Sprint 8.2's adversarial test set is a **standing regression suite**, re-run on every subsequent change to the `AI` module's tools, prompts, or guardrail logic — treated with the same permanence as IR-6/IR-7. |
| IR-11 | **Third-party API breaking changes** (Meta WhatsApp Cloud API, OpenAI API, Stripe API each version independently) | Medium — outside this project's control, can surface at any point post-launch | Pin API versions explicitly where each provider supports it; Milestone 11's monitoring/alerting (Sprint 11.1) is configured to surface integration error-rate spikes quickly, treating "detect fast" as the primary mitigation for a risk that can't be eliminated outright. |
...
**Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** Milestone 4 re-estimated as a single, smaller sprint (its actual as-built scope); Milestone 5 carries the original two-sprint "Salon Management" estimate this table previously assigned to Milestone 4 (its catalog-data scope is unchanged, only renumbered).
...
| 4. Salon Management | 1 | 2 days | 3 days | 5 days |
| 5. Employees, Services, Customers & Files | 2 | 6 days | 9 days | 14 days |
| 6. Scheduling Engine | 2 | 8 days | 12 days | 18 days |
| 7. WhatsApp Integration | 2 | 7 days | 11 days | 18 days *(includes IR-1 buffer)* |
| 8. AI Assistant | 2 | 8 days | 13 days | 20 days |
| 9. Billing | 2 | 6 days | 9 days | 14 days |
| 10. Analytics, Notifications & Admin | 2 | 6 days | 9 days | 14 days |
| 11. Production Deployment | 2 | 5 days | 8 days | 12 days |
| **Total (working days)** | **19** | **~61 days** | **~94 days** | **~147 days** |
| **Total (calendar time, 5-day weeks, ~85% focus-time realism factor)** | | **~14 weeks (~3.3 months)** | **~22 weeks (~5.1 months)** | **~35 weeks (~8 months)** |

### 11.3 Reading This Table

- **Optimistic** assumes every sprint's design was correctly anticipated by the prior source documents with minimal rework, no significant AI-integration or third-party-API surprises, and no external lead-time blocking (best-case IR-1/IR-5/IR-11).
...
- **Milestone 7 and 8 carry this roadmap's widest optimistic-to-conservative spread**, deliberately — they are simultaneously the highest-technical-complexity milestones (Section 4's own "Extra-Large" complexity ratings) and the two most exposed to genuine external unknowns (Meta's review process, OpenAI's real-world conversational behavior) that no amount of upfront documentation can fully de-risk in advance.
- This timeline explicitly **excludes** any post-launch roadmap work (PROJECT_REQUIREMENTS.md Section 11's Future Features) — it covers Milestones 1–11 only, ending at Section 4 Sprint 11.2's go-live, not an ongoing maintenance/growth timeline.
...
This document originally defined the **implementation roadmap and process only**, before any application code existed. Milestones 1–4 are now complete (see each milestone's own execution notes in Section 4 and their respective ADRs) — this section is retained as the historical planning record, amended below rather than rewritten, per this document's own living-document policy (Section 8.1).

**Amended, Milestone 4 (docs/adr/ADR-007-salon-management.md):** the original single "Salon Management" milestone was split into a profile-only Milestone 4 and a catalog-data Milestone 5, cascading every subsequent milestone's number up by one. The key decisions below are updated to reflect the resulting eleven-milestone, 19-sprint structure.

**Key decisions made in this phase, requiring explicit sign-off before each milestone's implementation:**
1. Eleven milestones, 19 sprints, in the dependency order justified in Section 1.1 — Foundation → Auth → Multi-Tenancy → Salon Management → Employees/Services/Customers/Files → Scheduling → WhatsApp → AI → Billing → Analytics/Notifications/Admin → Production.
...
3. A living-document policy making PRISMA_SCHEMA.md/API_SPECIFICATION.md/FRONTEND_ARCHITECTURE.md/docs/SALON_ARCHITECTURE.md updates a hard, PR-blocking requirement (Section 8.1/13), not a best-effort intention.
...
5. A 19-sprint / ~5.1-month expected timeline (Section 11), with Milestones 7 and 8 carrying the widest uncertainty range due to genuine external dependencies (Meta verification, OpenAI behavior) no amount of upfront planning fully eliminates.
6. Known gaps from prior documents (`POST /files`, `ServiceCategory` CRUD) are explicitly scheduled for closure in Sprint 5.1; invitation acceptance closed in Sprint 2.2, rather than left open indefinitely.

**Next work session should begin at whichever milestone/sprint is next incomplete per Section 4** — as of this amendment, that is Milestone 5, Sprint 5.1.
