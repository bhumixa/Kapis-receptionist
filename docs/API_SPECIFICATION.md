...
**Built Milestone 7 (docs/adr/ADR-010-whatsapp-platform.md)** — as-built, with these deviations from the design below: `POST /messages/send`'s body is `{ conversationId, body }` (no `mediaId` — outbound media sending isn't built; a manual reply is text-only) and its authorization is `STAFF`-broad, not scoped to conversation assignment (no AI/escalation exists yet to make that scoping meaningful — see the `Conversations` note next); `Conversation`/`Message` authorization for every endpoint below is `OWNER`/`MANAGER`/`STAFF` with no per-conversation assignment scoping (same reason); `ConversationStatus` is `OPEN`/`RESOLVED`/`CLOSED` only, so `filter[status][in]=ESCALATED,HUMAN_HANDLING` does not apply — there is no AI/escalation hand-off queue yet (Milestone 8). A new `GET/POST/DELETE /whatsapp/account` trio (not originally in this section) handles connecting/disconnecting the tenant's WhatsApp Business number, gated by a new `whatsapp:manage` permission (`OWNER`/`MANAGER`); `POST /whatsapp/account` verifies the supplied credentials against Meta before persisting (`400 INVALID_WHATSAPP_CREDENTIALS` on rejection), `409 ACCOUNT_ALREADY_CONNECTED`/`409 PHONE_NUMBER_ID_ALREADY_IN_USE` on conflict. Full as-built reference: docs/WHATSAPP_ARCHITECTURE.md, docs/MESSAGING_ARCHITECTURE.md.

**Amended Milestone 8 (docs/adr/ADR-011-ai-receptionist.md)** — `ConversationStatus` gains exactly one value, `ESCALATED` (no `HUMAN_HANDLING`; `Conversation.assignedUserId` distinguishes queued-unclaimed from claimed — see the ADR's narrowing rationale), so `filter[status]=ESCALATED` now applies. A new `PATCH /conversations/:id/assign` endpoint (body: `{ "userId": "uuid" | null }`, `STAFF`-broad, `TenantActiveGuard`) is the human-takeover/unassign action, delegating to `ConversationsService.assignUser` (built Milestone 7 but never exposed over HTTP until now). `Conversation` responses gain `escalatedAt`/`escalationReason`; `Message` responses gain `aiPromptVersion`.

Tag: `WhatsApp`. Base paths: `/api/v1/webhooks`, `/api/v1/messages`, `/api/v1/conversations`, `/api/v1/whatsapp`.
...
#### `PATCH /conversations/:id/assign` *(Milestone 8, docs/adr/ADR-011-ai-receptionist.md)*
**Purpose:** The human hand-off "take over" / "unassign" action — claims (or releases) an `ESCALATED` conversation for the calling staff member, distinct from `PATCH /conversations/:id`'s status transitions (FRONTEND_ARCHITECTURE.md Section 6.6's "Human takeover controls").
**Auth:** Required. **Authorization:** `OWNER`, `MANAGER`, `STAFF` — claiming a handed-off conversation is normal front-desk work, matching this section's existing STAFF-broad convention.
**Path Params:** `id`. **Request Body:** `{ "userId": "uuid" | null }` — `null` (or omitted) unassigns.
**Success — 200 OK:** `{ "success": true, "data": ConversationDTO }` — `status` is unchanged by this call (assignment and status are deliberately orthogonal; see the ADR).
**Errors:** `404 NOT_FOUND`.
**Rate Limit:** Standard-Authenticated. **Idempotency:** N/A.
...
**Built Milestone 8 (docs/adr/ADR-011-ai-receptionist.md)** — as-built, with these deviations from the design below: `checkAvailability`/`recommendService` have **no** HTTP endpoint (only `book`/`reschedule`/`cancel`/`faq` do) — read-only enough that the testable-HTTP-surface rationale below doesn't apply; they're called in-process only. `POST /ai/chat`'s `customerPhoneNumber`-based resolution is **not built** — `channel: "whatsapp"` requires an already-resolved `conversationId`, matching the in-process production path's own precondition (real traffic never calls this endpoint at all — see the architectural note immediately below). The documented `503 UPSTREAM_UNAVAILABLE` on `POST /ai/chat` does not occur in practice: an OpenAI failure always resolves to `200 OK` with the fallback message and `meta.degraded: true`, never a hard failure to the caller — the "Errors" line below is retained for historical accuracy against the pre-implementation draft but should be read as "handled via the fallback path, not a real response code." Every endpoint here (except `POST /ai/tools/faq`, which has no side effect) is verified end-to-end in `backend/test/integration/ai/*.integration-spec.ts`, including tenant isolation and human hand-off. Full as-built reference: docs/AI_ARCHITECTURE.md, docs/TOOLS.md, docs/PROMPT_ENGINEERING.md.
