...
- **Roles:** `SUPER_ADMIN`, `OWNER`, `MANAGER`, `STAFF` (PRISMA_SCHEMA.md `RoleName`). Each endpoint's "Authorization Required" field names the minimum role(s); `SUPER_ADMIN` is never implicitly included in a tenant-scoped endpoint's allowed roles (a Super Admin uses the separate `/admin/*` surface, Section 16, to act on tenant data, preserving the architectural separation from SYSTEM_ARCHITECTURE.md Section 8.4).
