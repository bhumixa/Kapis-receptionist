...
**Implementation note (Milestone 2 Core Authentication sprint, docs/adr/ADR-003-core-authentication.md):** as of this sprint, register redirects to `/auth/login?registered=true` (banner: "Account created. Please log in.") instead of the above — `/app/onboarding` doesn't exist until Milestone 3, and register issues no session to redirect into it with regardless (API_SPECIFICATION.md Section 4's register response has never included tokens). The behavior described above is the intended target once onboarding and email verification both exist; this note should be removed and the paragraph above revised once that lands.
...
> **Built Milestone 8 (docs/adr/ADR-011-ai-receptionist.md)** — as-built, with these deviations: the status filter is a single `ESCALATED` chip, not `ESCALATED,HUMAN_HANDLING` (see ADR-011's `ConversationStatus` narrowing) — filter chips are `All`/`Escalated`/`Open`/`Resolved`/`Closed`, not a segmented `in` filter. The AI-vs-staff visual marker is a small uppercase badge ("AI"/"Staff") rendered above the message content, plus a distinct indigo bubble color for AI messages. The AI Context panel is gated behind `!environment.production` (a genuine dev-only surface, not just collapsed-by-default) and reads `GET /ai/context/:conversationId`. "Take over"/"Unassign" buttons appear on an `ESCALATED` conversation's header, calling the new `PATCH /conversations/:id/assign`.
...
> **Built Milestone 8 (docs/adr/ADR-011-ai-receptionist.md)** — as-built, with these deviations: sections are stacked on one page, not tab-switched (matching this page's existing Milestone-3-onward pattern, not a Milestone 8 change). "AI Behavior" edits `general.ai.*` (enabled toggle, tone, greeting message with live WhatsApp-bubble preview, escalation instructions, fallback message, repeated-clarification escalation threshold) via `PATCH /tenant/settings`, deliberately excluding that key from the raw-JSON "General" textarea above it so the two editors never race (docs/AI_ARCHITECTURE.md §2/ADR-011). Also added, not in the original design: a read-only **Prompt Versions** table (`GET /ai/prompt-versions`) and a **Test my AI** sandbox (a text input + response bubble calling `POST /ai/chat` with `channel: "dashboard_test"` — no live typing indicator, since no WebSocket/SSE transport exists; a loading state on the Send button is the "AI activity indicator" for this surface). "Booking Policy" is not built this milestone (Milestone 6's `business` namespace remains raw JSON).
